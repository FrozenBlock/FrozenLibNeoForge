...
public abstract class Config<T> {
	...
	private void invokeSaveEvents() {
		String formatted = this.formattedName();
		try {
			NeoForge.EVENT_BUS.post(new ConfigEvent.Save(this));

			if (FMLLoader.getDist().isClient()) {
				NeoForge.EVENT_BUS.post(new ConfigEvent.Save.Client(this));
			}
		} catch (Exception e) {
	...
	private void invokeLoadEvents() {
		String formatted = this.formattedName();
		try {
			NeoForge.EVENT_BUS.post(new ConfigEvent.Load(this));

			if (FMLLoader.getDist().isClient()) {
				NeoForge.EVENT_BUS.post(new ConfigEvent.Load.Client(this));
			}
		} catch (Exception e) {
